...
C 4 - REAL (NOT IMPLIMENTED)
C 5 - COMPLEX (NOT IMPLIMENTED)
C 6 - DOUBLE PRECISION (NOT IMPLIMENTED)
C 7 - DOUBLE COMPLEX (NOT IMPLIMENTED)
...
C GOTO
 4000 LINE = ADJUSTL(LINE(5:))
    ...
    4 WRITE(TMPSTR, *) LINEN
      WRITE (*,*) 'SYNTAX ERROR ON LINE ',  ADJUSTL(TMPSTR)
      CLOSE (8)
      STOP
...
30000 TEMP = TEMP + 1
      IF (TEMP.EQ.101) GOTO 3202
      TEMP2 = OUTPUT(TEMP)
      IF (TEMP2.EQ.1000000123) GOTO 3202
...
30699 WRITE (TMPSTR, *) SP
      WRITE (9, '(A)') 'PSH '//TRIM(ADJUSTL(TMPSTR))
      GOTO 30000
30700 WRITE (9, '(A)') 'POP R1', 'LLOD R2 SP R1', 'LSTR SP R1 @MAX',
     1'ADD R1 R1 R2', 'PSH R1', 'MOV R2 SP', 'LOD R1 SP',
     2'BRE ~+3 R1 @MAX', 'INC SP SP', 'JMP ~-3', 'LLOD R1 SP -1',
     3'STR SP R1', 'DEC SP SP', 'BNE ~-3 SP R2', 'INC SP SP'
      GOTO 30000
30800 TEMP = TEMP + 1
      TEMP2 = OUTPUT(TEMP)
      SP = TYPES(TEMP2)
...
C    POP R1 //2
C    LOD R3 M(VARADR) //5
C    IMM R2 M(VARADR+1) //M1
C    ADD R4 SP R1 //begining of string //SP+2
C    INC SP R4
C    CPY R2 R4
C    DEC R3 R3
C    DEC R4 R4
